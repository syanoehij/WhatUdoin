...
| (c) M1b 50 VU smoke 명확한 p95 회귀 없음·`database is locked` 0건 유지 | PASS (2026-05-10 갱신) | locust payload 누락 필드(`description`/`location`/`all_day`) 수정 후 재측정: PUT 0% 실패, p95 3000ms(baseline 5300ms 대비 개선). lock 0건. `성능 개선 진행 결과(M1b).md` `M1b-U5 closure — PASS` 절 참조 |
...
**M1-ULTRA 종료 판정**: lite 경로 6종 **전부 PASS** (c항 2026-05-10 재측정으로 부분 PASS → PASS 갱신). M1-ULTRA 사이클 종료.
...
- **M1b-U5 p95 closure**: 완료 (2026-05-10). `app.py update_event` setdefault 보강 + locustfile 수정으로 해결.
