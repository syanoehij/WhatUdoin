...
|------|----|-----------|---------|
| image | True | 0 | 10 |
| attachment | True | 15 | 20 |

| 항목 | 결과 | 비고 |
|------|------|------|
| app.py import 성공 | PASS |  |
| 일반 API p95 < 500.0ms (업로드 중) | PASS | p95=93.0ms (20MB/10MB 혼합 업로드 thread 진행 중 측정) |
| 일반 API 응답 도달 50/50 | PASS | reached=50 |
| RSS 스파이크 < 100MB (업로드 중 50회 GET 기준) | PASS | delta=2.6MB |
| Mock image 업로드 성공 | PASS | elapsed=0ms, error= |
| Mock attachment 업로드 성공 | PASS | elapsed=15ms, error= |
| 10MB 이미지 처리 완료 (60s 이내) | PASS | elapsed=0ms |
| 20MB 첨부파일 처리 완료 (60s 이내) | PASS | elapsed=15ms |
