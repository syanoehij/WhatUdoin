...
- **다음 행동**: M2-16 완료. `publisher.py` publish 추상화 + `sse_service.py` Starlette SSE service 신설, app.py 37개 publish 호출 일괄 교체, supervisor sse_service_spec factory 추가. probe 19/19 + phase58 19/19 PASS, phase54~57 회귀 모두 PASS. 다음은 M2-17 내부 토큰 인증 + healthcheck + 로그 분리 + crash-loop.
