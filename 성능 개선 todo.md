...
- **다음 행동**: M2-19 완료. ASGI dispatcher 구조에서 SSE proxy 6종 조건이 자연 보장됨을 단언으로 잠금(buffering/compression/idle timeout/disconnect/header/external internal block). 운영 코드 변경 0건. probe 31/31 + phase61 31/31 PASS, phase54~60 회귀 PASS. 다음은 M2-20 종료 부하 테스트 + 정책별 증거 인덱스.
...
| M2 | target-risk override | M2-20 대기 | **20/21** | M2-1 외부 포트 소유자: Tray/Supervisor/Front Router. M2-2 SSE 접근 경로: (a). M2-3 canonical URL: PC LAN IP 기반. M2-4 HTTP fallback unsafe write 차단. M2-5 HSTS 미적용. M2-6 AVR 사용 배포. M2-7 SSE 별도 LAN 포트 비채택. M2-8 frozen self re-spawn PASS. M2-9 Supervisor skeleton. M2-10 minimal Front Router. M2-11 forwarded strip-then-set. M2-12 limiter trusted-proxy key. M2-13 TRUSTED_PROXY + Web API direct-access boundary. M2-14 CSRF Host 보존 채택(코드 변경 0). M2-15 HTTPS probe scheme 보존 채택(코드 변경 0). M2-16 SSE broker → SSE service 프로세스 이전 + publish IPC 추상화. M2-17 토큰 인증 + healthz + crash-loop 차단. M2-18 publish 실패 정책(backend 카운터/로그/healthz + frontend idle visibility 재조회). M2-19 SSE proxy 6종 자연 보장(코드 변경 0) |
