...
- **다음 행동**: M2-17 완료. SSE service 토큰 검증(timing-safe + raw 0건) + `/healthz` SSE/Web API 양쪽 + supervisor 5분 3회 crash-loop 차단(`degraded` 격상 + reset 헬퍼) + `probe_healthz` 헬퍼. probe 22/22 + phase59 22/22 PASS, phase54~58 회귀 모두 PASS. 다음은 M2-18 publish 실패 유실 정책.
