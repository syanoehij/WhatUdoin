...
- **다음 행동**: **M6 후보 마일스톤 종료 (2026-05-10)**. M6-0~M6-4 5개 step 완료. M6-4 정책별 증거 인덱스 11종 + 게이트 8/8 PASS. phase54~75 22개 회귀 PASS. 운영 코드 변경 0건. **모든 마일스톤(M1a/M1b-ULTRA/M1c-ULTRA/M1d/M1-ULTRA 종료/M2/M3/M4/M5/M6) 종료**. 회사 반입 결정 또는 §18 후속 후보(보수 단축안/MCP write tool 도입/DB command service) 별도 사이클로 처리.
...
| step | 제목 | 모델 | §참조 | exit criteria |
|------|------|------|-------|---------------|
| [x] M1-end-U1 | M1-ULTRA 종료 점검 | Opus | §17 M1 종료 부하 테스트 (축소) | **종료 (2026-05-09, U5 closure 2026-05-10 반영)** — (a) M1a lazy-load 회귀 0건 + before/after 기록 (M1a-11/12 evidence) / (b) M1b WAL 적용 후 `journal_mode=wal` 확인 (M1b-U4) / (c) M1b 50 VU smoke `database is locked` 0건 유지 + 재측정 p95 3000ms(baseline 5300ms 대비 개선) (M1b-U5 closure PASS) / (d) M1c Ollama 기본 1슬롯 limiter 동작 (M1c-U5: capacity=1 시 1개 200 + 4개 503 reason="busy") / (e) 동시 AI 2개+ smoke 포화 응답 확인 (M1c-U5: N=5 fire 결과 + 1→3 resize live 반영) / (f) M1d skip 사유 기록 완료. 상세 기록: `성능 개선 진행 결과(M1c).md` |
...
| step | 제목 | 모델 | §참조 | exit criteria |
|------|------|------|-------|---------------|
| [x] M2-0 | M2 진입 게이트 평가 | Opus | §13 진입 게이트 — SSE service (M2) | **target-risk override 진입 (2026-05-10)** — 개발 PC 조회/SSE/restart probe는 NO-GO(`/api/events` p95: baseline 89.8ms, 50 SSE 89.3ms, restart reconnect 88.3ms, failure 0건). 단, 회사 서버 저사양 + HDD/NASCA I/O 리스크로 개발 PC 측정을 운영 NO-GO로 일반화하지 않고 M2 진입. 상세: `성능 개선 진행 결과(M2).md` |
| [x] M2-1 | 외부 포트 8000/8443 listener 소유자 결정 | Opus | §16 M2 0번 + §13 Tray/Supervisor + Front Router | **Tray/Supervisor/Front Router 소유로 결정 (2026-05-10)** — 기존 외부 8000/8443 listener, TLS/인증서, 라우팅, service lifecycle, watchdog은 Front Router/Supervisor가 소유. main app은 내부 Web API service로 전환하는 장기 구조를 채택. M2-2는 라우터 소유 결정과 정합되는 SSE 접근 경로를 선택해야 함 |
| [x] M2-2 | 사용자-facing SSE 접근 경로 (a)/(b)/(c) 결정 | Opus | §13 사용자-facing SSE 접근 경로 | **(a) 채택 (2026-05-10)** — Tray/Supervisor/Front Router가 외부 `/api/stream`을 SSE service로 라우팅. 브라우저는 기존 origin 상대 경로 유지, SSE service는 내부 loopback bind 가능. (b) 별도 LAN 포트는 CORS/쿠키/인증서/CSP 비용으로 비채택, (c) main app proxy는 재시작 시 SSE 유지 목표가 약해져 비채택. 상세: `성능 개선 진행 결과(M2).md` |
| [x] M2-3 | 외부 canonical URL 정책 결정 | Opus | §13 외부 canonical URL 정책 | **PC LAN IP 기반 canonical origin 채택/적용 (2026-05-10)** — Supervisor/Front Router가 `WHATUDOIN_PUBLIC_BASE_URL=https://<PC-IP>:8443`를 주입하는 단일 소유 구조. Web API는 env host를 우선 사용하고 없으면 request host fallback. `/settings/mcp`, MCP 가이드, 인증서 probe, AVR fallback, 체크/문서 링크 복사 UI 적용. `request.base_url*` 직접 사용 0건, hostname:8000 템플릿 직접 조합 0건. 상세: `성능 개선 진행 결과(M2).md` |
| [x] M2-4 | HTTP 8000 fallback 인증/쓰기 범위 결정 + baseline 표 | Opus | §13 HTTP 8000 fallback 인증/쓰기 범위 | **권장안 채택/적용 (2026-05-10)** — HTTP 8000 fallback unsafe write 차단 + AVR whitelist 예외(`/avr`, `/remote`, `/api/avr`, `/api/avr/*`). Pre-policy baseline: `/api/events` POST, `/api/doc/{id}` PUT, `/api/checklists` POST, `/api/upload/image` POST, `/api/upload/attachment` POST 모두 200. Post-policy: 동일 unsafe write 모두 403, `/api/events` GET 200 유지. 상세: `성능 개선 진행 결과(M2).md` |
| [x] M2-5 | HSTS 적용 여부 결정 | Opus | §13 HSTS 적용 여부 결정 | **HTTP fallback 유지 중 HSTS 미적용 채택 (2026-05-10)** — M2-4 이후 HTTP unsafe write는 차단됐지만, 인증서 안내/probe fallback·AVR 예외·non-write fallback 경로가 남아 있어 `Strict-Transport-Security`를 적용하지 않는다. 실제 8443/8000 응답 모두 HSTS absent 확인. 상세: `성능 개선 진행 결과(M2).md` |
...
| [x] M2-10 | Front Router 최소 구현 | Opus | §13 외부 라우팅 표 | **Minimal ASGI router 적용 (2026-05-10)** — `front_router.py`에서 `/api/stream`→SSE app, `/`/`/api/*`→Web API app, `/uploads/meetings/*`→Web API app, `/internal/*` 404 차단을 구현. `expose_route_headers`는 기본 비활성. isolated probe PASS. 실제 SSE broker 이전은 M2-16, SSE proxy 조건은 M2-19로 유지. 상세: `성능 개선 진행 결과(M2).md` |
...
| [x] M2-15 | HTTPS probe middleware 라우터 호환 | Opus | §13 Front Router HTTPS probe middleware 호환 조건 | **scheme 보존(=재구성과 동등) 채택/적용 (2026-05-10)** — Front Router는 ASGI dispatcher라 forwarded scope build가 `scope["scheme"]`을 변경하지 않고 sub-app에 그대로 forward한다. 외부 https → downstream scheme=https → middleware 통과(probe HTML 0건). 외부 http → scheme=http → middleware 동작 → probe HTML. probe 23/23 PASS, phase57 회귀 23/23 PASS. forwarded scope scheme 보존 / https inbound downstream 도달 + probe 0건 / http 브라우저 navigate probe HTML / wd-cert-skip cookie 우회 / `/api*` skip prefix 우회 / HEAD 우회 / CORS non-navigate 우회 / 라우터 미경유 직접 http 8000 동일 동작. 상세: `성능 개선 진행 결과(M2).md` |
| [x] M2-16 | SSE broker SSE service 프로세스로 이전 | Opus | §13 채택 — SSE service 분리 + main → SSE 통신 | **적용 완료 (2026-05-10)** — `publisher.py` publish 추상화(WHATUDOIN_SSE_PUBLISH_URL 환경 분기, stdlib urllib.request, silent fail + sse_publish_failure 카운터, Bearer 토큰 자리 마련), `sse_service.py` Starlette ASGI app(GET /api/stream + POST /internal/publish loopback guard + 자체 SSEBroker), app.py 37개 wu_broker.publish → _sse_publish 일괄 교체 + WHATUDOIN_SSE_SERVICE_URL 설정 시 /api/stream 503, supervisor.sse_service_spec() factory(loopback bind + 포트 보호 env). probe 19/19 + phase58 19/19 PASS, phase54~57 회귀 PASS. 토큰 강제·healthcheck·crash-loop은 M2-17, 카운터 expose는 M2-18 범위. 상세: `성능 개선 진행 결과(M2).md` |
| [x] M2-17 | 내부 토큰 인증 + healthcheck + 로그 분리 + crash-loop | Opus | §13 supervisor lifecycle + watchdog + §14 watchdog | **적용 완료 (2026-05-10)** — sse_service `/internal/publish`에 secrets.compare_digest timing-safe 토큰 검증, env 미설정 모두 401, 토큰 raw 값 로그 0건. SSE service `/healthz` + Web API `/healthz` + `_SKIP_EXACT` 등록. supervisor에 5분 윈도우 / 3회 임계 crash-loop 차단(`crash_history`/`_record_crash`/`_is_crash_loop`/`reset_crash_loop`), 진입 시 status=`degraded` + spawn 차단. `probe_healthz` 헬퍼 정의(watchdog 폴링은 M2-20). protected env에 INTERNAL_TOKEN 추가. probe 22/22 + phase59 22/22 PASS, phase54~58 회귀 PASS. 상세: `성능 개선 진행 결과(M2).md` |
| [x] M2-18 | publish 실패 유실 정책 적용 | Opus | §13 publish 실패 유실 정책 | **적용 완료 (2026-05-10)** — backend: publisher.py에 logger + threading.Lock + deque(maxlen=50) `_failure_meta`, `_record_failure` 헬퍼, `get_failure_snapshot()`. token raw 값 로그 0건. app.py /healthz에 `sse_publish_failures`/`last_event`/`last_at` 3개 키 추가. publisher.publish silent (raise 0). in-process fallback broker 예외 isolation. frontend: realtime.js (a) 재연결 재조회 보존 + (b) idle visibility/focus 재조회(IDLE_REFETCH_THRESHOLD_MS=60000, MIN_REFETCH_INTERVAL_MS=5000, click/keydown/scroll/mousemove passive listener, wu:events:changed 1회 dispatch, `__wuRealtimeDebug` 노출). probe 17/17 + phase60 22/22 + phase59 25/25 PASS, phase54~58 회귀 PASS. 상세: `성능 개선 진행 결과(M2).md` |
| [x] M2-19 | Front Router SSE proxy 6종 조건 적용 | Opus | §13 Front Router SSE proxy 조건 | **6종 자연 보장 (코드 변경 0건, 2026-05-10)** — ASGI dispatcher 구조에서 buffering(StreamingResponse 즉시 forward + Cache-Control no-cache + X-Accel-Buffering no)/compression(5개 파일 GZip 등 grep 0)/idle timeout(asyncio.wait_for 0 + SSE 25초 heartbeat 코드)/disconnect(request.is_disconnected break + finally unsubscribe)/header(FORWARDED_HEADER_NAMES에 cookie/authorization 미포함, X-Forwarded-* 6종만 재작성)/external internal block(M2-10 match_front_route blocked + 404 + downstream 0)이 모두 단언으로 잠겼다. probe 31/31 + phase61 31/31 PASS, phase54~60 회귀 PASS. 라이브 브라우저 EventStream/50 SSE 연결 지연/curl `/internal/*` 외부 호출은 M2-20 라이브 보강. 상세: `성능 개선 진행 결과(M2).md` |
| [x] M2-20 | M2 종료 부하 테스트 + 정책별 증거 인덱스 작성 | Opus | §17 M2 완료 기준 | **M2 종료 (2026-05-10)** — 정책별 증거 인덱스 18종 + 추가 보안 2종 표 작성 완료. 라이브 supervisor 통합 probe(SSE service subprocess.Popen → /healthz → token 3종 → stop_all) 14/14 PASS. 50 SSE 부하 시뮬레이션 10/10 PASS(SSE 연결 50/50/219ms, event 도달 50/50, publish→수신 p95 <0.1ms loopback fan-out, subscribers 0 복귀, /healthz 50회 동시 GET p95 16.0ms). 운영 코드 변경 0건. phase54~61 회귀 155단언 모두 PASS. M3 진입 게이트 평가는 별도 step으로 분리. 상세: `성능 개선 진행 결과(M2).md` | — 다음 항목 각자에 (증거 파일 경로 / 로그 발췌 / Playwright 또는 회귀 테스트명) 3종을 표로 기록. **라우팅/보안 11종**: ① 외부 포트 소유자(M2-1), ② 사용자-facing SSE 경로 결정과 측정(M2-2), ③ canonical URL 적용 범위 — `request.base_url.hostname` 직접 사용 0건 grep 결과 + `/settings/mcp` 응답에 외부 canonical URL + 내부 loopback/`8000` 0건(M2-3), ④ HTTP 8000 fallback baseline 표 + 채택 정책에 부합하지 않는 응답 0건(M2-4), ⑤ HSTS 적용/미적용 결정과 max-age(M2-5), ⑥ AVR scope-out 또는 회귀 테스트 5종 통과(M2-6), ⑦ strip-then-set 위조 헤더 3종 동시 주입 회귀(M2-11), ⑧ SlowAPI limiter 50 VU 분리 bucket + 위조 무시 + 분당 11회 429 회귀(M2-12), ⑨ TRUSTED_PROXY + 외부 직접 차단 3종(M2-13), ⑩ CSRF Host 보존 + Origin 위조 차단(M2-14), ⑪ HTTPS probe middleware 라우터 경유 페이지 probe HTML 0 + 외부 직접 8000 회귀 0(M2-15). **Supervisor/lifecycle 4종**: ⑫ Supervisor 상태 표시 — Web API/SSE service의 pid·health·restart_count·last_error·log path 모두 표시/기록(M2-9·M2-17), ⑬ service crash 1회 자동 재시작 + 5분 3회 crash-loop 차단/degraded 전환 검증(M2-17), ⑭ `sse_publish_failure` 카운터 동작 + SSE service 강제 종료/재시작 시 카운터만 증가하고 정상 흐름 0건(M2-18), ⑮ DB rollback 금지 — publish 실패 시 사용자 write가 DB commit 정상 + SSE 재연결 시 화면 재조회로 최종 상태 일치(M2-18). **Build·Infra·SSE proxy 3종**: ⑯ PyInstaller frozen 환경 self re-spawn 빌드 검증 통과 — onedir 빌드 산출물에서 `sys.executable --service=<name>` spawn, multiprocessing freeze_support, 단일 EXE 종료 신호로 모든 service graceful 종료(M2-8 결과), ⑰ main app(Web API) → SSE service publish 채널 내부 토큰 인증 — 토큰 없는 `/internal/publish` 호출 401, 잘못된 토큰 401, 정상 토큰 200 회귀 테스트(M2-17), ⑱ Front Router SSE proxy 6종 조건 — buffering 비활성/compression 미적용/idle timeout이 SSE heartbeat보다 길게 설정/client disconnect 시 upstream close 전파/session cookie·Authorization·X-Forwarded-For 통과/외부 `/internal/*` 차단을 (a) 실제 브라우저(Chrome/Edge) network trace EventStream 탭, (b) 50 SSE 연결 스크립트 평균 publish→수신 지연, (c) `curl https://host/internal/publish` 외부 호출 404/403 응답 — 3종 증거로 모두 확인(M2-19). **추가 보안 검증**: MCP handshake 실패/access log에 `Authorization` raw 값과 Bearer token 원문 0건 grep(§17 M2 완료 기준), `/uploads/meetings/*` 비멤버 권한 검사 회귀(M2-10) |
...
| [x] M3-3 | healthcheck/로그/graceful shutdown 검증 | Sonnet | §13 watchdog + §14 graceful shutdown | **적용 완료 (2026-05-10)** — scheduler healthz 응답 4종 키 추가(jobs_count/next_run_at/last_finalize_expired_done_at/uptime_seconds, starting/degraded 분기 200 유지). RotatingFileHandler 10MB×14 backup + delay=True(Windows 락 방지) `scheduler.app.log` + WHATUDOIN_SCHEDULER_LOG_DIR override + apscheduler/root 로거 양쪽. supervisor STOP_ORDER=(ollama/sse/scheduler/web-api) + stop_all 재작성(0.5s grace × N service, 미존재 skip). M2_STARTUP_SEQUENCE 8종 정합. probe 35/35 + phase64 31/31 PASS, phase54~63 회귀 PASS. 라이브 통합은 M3-4. 상세: `성능 개선 진행 결과(M3).md` |
| [x] M3-4 | M3 종료 부하 + exit criteria 점검 | Opus | §17 M3 완료 기준 | **M3 종료 (2026-05-10)** — 라이브 supervisor 통합(scheduler_service.py subprocess.Popen → healthz jobs_count=6/last_finalize 채워짐 → stop_all 0.50s 내) 14/14 PASS. owner 정책 위반 3종 0건(finalize_expired_done 합산 1건/알림 dedup 가드/백업 trigger 분리 + sqlite atomic backup) 21/21 PASS. 일반 API p95 12.9ms(SLA 500ms 대비 38배 여유) + database is locked 0건 8/8 PASS. phase65 35/35 + phase54~64 회귀 모두 PASS. 운영 코드 변경 0건. 상세: `성능 개선 진행 결과(M3).md` |
...
| [x] M4-2 | M1c-9 통합 UX의 service 분리 후 유지 + 강제 종료/hang 검증 | Sonnet | §8 Ollama 서버 장애 통합 처리 + §17 M4 완료 기준 | **회귀 잠금 완료 (2026-05-10)** — 통합 UX 5종 분기(connect/timeout/busy+slots/5xx + env 양쪽 동일 OllamaUnavailableError) phase67 33/33 PASS. 라이브 supervisor lifecycle probe(spawn → IPC → SIGTERM → unavailable UX → 재시작 회복) 19/19 PASS. mock hang probe(IPC timeout → reason="timeout" + "AI 사용 불가") 4/4 PASS. 운영 코드 변경 0건. uvicorn threadpool 격리 라이브 회귀는 M4-4. 상세: `성능 개선 진행 결과(M4).md` |
| [x] M4-3 | uvicorn `limit_concurrency` 최종 결정 | Opus | §12 sidecar 도입 여부에 따른 분기 | **미적용 채택 (2026-05-10)** — (가) sidecar 도입 후 분기 후보 "초기 미적용" 채택. M2~M4 sidecar 분리로 main app threadpool 잠식 시나리오 0, 사내 n≈1~5명 폭주 미관측, §8 Ollama 세마포어 + §7 업로드 세마포어 + WAL/busy_timeout으로 충분. SSE service §12 명시 미적용(503 차단 시 알림 정합성). limit_max_requests도 §12 금지로 미적용. uvicorn 호출 10건 grep 0건. phase68 3/3 PASS. 재평가 트리거: 동시 사용자 50+, p95 회귀, threadpool 잠식 측정 시. 상세: `성능 개선 진행 결과(M4).md` |
| [x] M4-4 | M4 종료 부하 + exit criteria 점검 | Opus | §17 M4 완료 기준 | **M4 종료 (2026-05-10)** — 라이브 4 service 통합 probe(ollama/sse/scheduler 라이브 spawn + web_api mock, STOP_ORDER stop_all 2.03s) 21/21 PASS. Ollama hang 중 일반 API p95 31.0ms(SLA 500ms 대비 16배 여유, 3 AI 워커 IPC timeout 동안 일반 asyncio 루프 격리) 8/8 PASS. phase69 39/39 PASS. phase54~68 회귀 모두 PASS. 운영 코드 변경 0건. M4 종료 게이트 5/5 PASS. 상세: `성능 개선 진행 결과(M4).md` |
...
| [x] M6-4 | M6 종료 회귀 + exit criteria | Opus | §17 M6 후보 완료 기준 | **M6 종료 (2026-05-10)** — 정책별 증거 인덱스 11종 작성. MCP write owner 원칙 재확인: SQL write 패턴 0/sqlite3 직접 import 0/db.write 호출 0(AST Constant + Call 검사) + `_call_web_api_command` IPC boundary 헬퍼 NotImplementedError 잠금. hidden-project/lock/history/SSE publish는 M2~M5 boundary 자연 보존(phase54~75 sweep). DB command service §18 후속 보존. 운영 코드 변경 0건. M6 종료 게이트 8/8 PASS. 상세: `성능 개선 진행 결과(M6).md` |
...
| M1b-ULTRA | M1a 완료 | **완료** | **5/5** | WAL/PRAGMA 1.5h 경로. U1~U5 완료. U5 closure PASS: locust payload 누락 필드 보강 후 재측정 p95 3000ms(baseline 5300ms 대비 개선), `database is locked` 0건 |
...
| M6 후보 | target-risk override | **완료** | **5/5** | M6-0~M6-3 적용 + M6-4 종료: 정책별 증거 인덱스 11종 + MCP write owner 원칙 재확인(SQL write/sqlite3 import/db.write 호출 0건 AST 검사) + 게이트 8/8 PASS. phase54~75 22개 회귀 PASS. 운영 코드 변경 0건. **M6 마일스톤 종료** |
...
- M1c/M1d/M1-end: [`성능 개선 진행 결과(M1c).md`](성능%20개선%20진행%20결과(M1c).md)
- M2: [`성능 개선 진행 결과(M2).md`](성능%20개선%20진행%20결과(M2).md)
- M3: [`성능 개선 진행 결과(M3).md`](성능%20개선%20진행%20결과(M3).md)
- M4: [`성능 개선 진행 결과(M4).md`](성능%20개선%20진행%20결과(M4).md)
- M5: [`성능 개선 진행 결과(M5).md`](성능%20개선%20진행%20결과(M5).md)
- M6: [`성능 개선 진행 결과(M6).md`](성능%20개선%20진행%20결과(M6).md)
