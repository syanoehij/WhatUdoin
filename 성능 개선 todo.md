...
- **다음 행동**: M3-3 완료. scheduler healthz 4종 키 확장(jobs_count/next_run_at/last_finalize_at/uptime), RotatingFileHandler 10MB×14 backup, supervisor STOP_ORDER 4종 + stop_all 재작성(0.5s grace × N). probe 35/35 + phase64 31/31 PASS, phase54~63 회귀 PASS. 다음은 M3-4 종료 부하 + exit criteria(라이브 supervisor 통합 + owner 정책 위반 3종 + database lock 0 + p95 500ms).
...
| M3 | target-risk override | M3-4 대기 | **4/5** | M3-0 target-risk override 진입. M3-1 owner 표. M3-2 scheduler_service.py + env 분기 + supervisor factory. M3-3 healthz 4종 키 + RotatingFileHandler 10MB×14 backup + supervisor STOP_ORDER 4종 + stop_all 재작성 |
