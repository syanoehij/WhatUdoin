...
- **다음 행동**: M3 진입 (target-risk override). M3-0 진입 게이트 평가 완료(정량 NO-GO지만 supervisor 재사용/owner 정책 코드화/야간 격리/사용자 명시 요청 사유). 다음은 M3-1 startup maintenance 단일 owner 표 확정.
...
| M3 | target-risk override | M3-1 대기 | **1/5** | M3-0 target-risk override 진입(M2-0과 동일 패턴): 정량 NO-GO지만 supervisor 재사용/owner 정책 코드화/야간 격리/명시 요청 사유 |
