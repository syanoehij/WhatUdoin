...
- **다음 행동**: M2-15 완료. Front Router ASGI dispatch가 `scope["scheme"]`을 변경하지 않고 sub-app에 forward하므로 외부 https는 middleware 통과·외부 http는 probe HTML 응답이 자연 보존된다. probe 23/23 + phase57 회귀 23/23 PASS, 운영 코드 변경 0건. 다음은 M2-16 SSE broker SSE service 프로세스 이전.
