...
- 작은 버그 수정, 작은 UI 수정은 planner 없이 오케스트레이터가 바로 구현/review/qa 흐름을 시작할 수 있다. 이 경우에도 subagent를 생성한다면 오케스트레이터가 최소 역할별 dispatch packet을 먼저 작성한다. 하네스 문서 자체의 작은 수정은 오케스트레이터가 직접 처리할 수 있다.
- 기능 추가, DB/API/UI가 함께 바뀌는 작업, 성능 개선, 권한/보안 변경, 범위가 애매한 작업은 먼저 planner subagent를 실행한다.
- planner는 구현하거나 다른 subagent를 직접 호출하지 않는다. 요구사항 분석, 실행 계획, 필요한 역할 판단, 역할별 dispatch packet만 산출한다.
- 실제 subagent 호출권과 중단/범위 변경 판단은 항상 Codex 본체 오케스트레이터가 유지한다.
- planner 완료 후 오케스트레이터는 planner가 만든 `.codex/workspaces/current/dispatch/*.md` 목록을 확인하고, 필요한 subagent를 생성한 뒤 각 subagent가 자기 dispatch packet을 읽게 한다. plannerless 흐름에서도 같은 경로의 최소 packet을 오케스트레이터가 만든다.
- 역할별 dispatch packet에는 최소한 담당 역할, 읽을 파일, 작업, 파일 소유권, 이번 작업 diff 범위, scope revision, 서버 lifecycle 정책, 성공 기준, 검증, 산출물 경로를 포함한다.
- backend-dev/frontend-dev 완료 후 오케스트레이터는 실제 변경 파일과 `*_changes.md`를 확인해 downstream code-reviewer/qa packet의 `Diff Scope`와 검증 초점을 갱신하거나 stale 여부를 명시한다.
- subagent는 메인 대화와 분리된 컨텍스트로 실행한다. 필요한 요구사항, 파일 경로, 이전 단계 산출물만 명시적으로 전달한다.
- 역할 간 공유 상태는 대화 기억이 아니라 `.codex/workspaces/current/*.md`와 `.codex/workspaces/current/dispatch/*.md` 산출물을 기준으로 한다.
...
- 서버는 사용자가 VSCode 디버깅 모드로 실행 중일 수 있다. 기본값은 User-managed이므로 코드 변경 뒤 서버 재시작이 필요하면 사용자에게 요청하고 직접 kill/restart하지 않는다.
- 예외: 사용자가 명시적으로 서버 실행/종료를 Codex에 위임했고 그 사실이 feature spec 또는 dispatch packet의 `Server Lifecycle: Codex-managed`에 기록된 경우에만 Codex가 서버 실행/종료를 관리할 수 있다. 이 경우 자신이 시작한 프로세스의 PID/log/port를 기록하고 종료 시 정리한다.
...
- `.codex/workspaces/current/dispatch_notes.md`: 오케스트레이터가 전체 dispatch 상태를 빠르게 파악하기 위한 요약
- `.codex/workspaces/current/dispatch/*.md`: planner가 작성한 역할별 지시 packet. 예: `backend-dev.md`, `frontend-dev.md`, `code-reviewer.md`, `qa.md`
