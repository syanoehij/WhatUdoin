...
- [x] **구현**
  - [x] 알림 통합 표시 쿼리: `team_id IN user_team_ids(user)` (승인 + 비삭제) — `auth.user_team_ids` 위임으로 deleted_at + approved 자동 적용.
  - [x] 추방·탈퇴된 팀의 알림은 화면 필터로 제외 (row는 유지) — `get_pending_notifications` viewer 분기.
  - [x] 알림 항목에 발생 팀 이름 표시 — LEFT JOIN teams + `notif-team` 뱃지.
  - [x] 알림 클릭 → `notifications.team_id`로 작업 팀 자동 전환 + toast — `onNotifClick` + 기존 `POST /api/me/work-team` 라우트(#15) 재사용.
  - [x] `team_id IS NULL` 알림은 작업 팀 전환 없이 알림 페이지에만 — onclick 핸들러 미부착.
- [x] **검증**
  - [x] 다른 팀 알림 클릭 → 작업 팀 전환 + 상세 화면 이동 (`location.reload()` 후 새 컨텍스트, B3 + S4 정적 검증).
  - [x] 추방된 팀의 과거 알림이 안 보임 (B2 — status='rejected' 시 user_team_ids 에서 제외).
  - [x] 재가입 후 그 팀 알림 다시 보임 (재가입 시 status='pending'→'approved' 로 갱신되어 user_team_ids 다시 포함 — 구조적 보장).
...
- [x] **구현**
  - [x] MCP read-only 도구 기본 조회 범위: 모든 승인·비삭제 팀 (`_mcp_work_team_ids = auth.user_team_ids` — 이전 사이클들 흡수)
  - [x] `team_id` 선택 파라미터 추가 (list_projects/list_events/list_documents/list_checklists 4개)
  - [x] 프로젝트 필터: `project` 문자열은 `team_id` 동반 시만 안전한 단일 프로젝트 해석 (docstring 안내)
  - [x] 결과에 `team_id`, `team_name` 포함 (`get_all_meetings_summary` 등 기존 헬퍼 LEFT JOIN teams)
  - [x] 일반 사용자/멤버 조회에서 admin 제외 (현재 14 tools 에 user-listing 도구 없음 — 흡수 메모)
  - [x] **admin의 MCP 토큰 발급 차단**: `templates/settings_mcp.html` admin 분기 + `POST /api/me/mcp-token/regenerate` 첫 줄 `is_admin → 403`
- [x] **구현 (히든 프로젝트 가시성 — 모든 채널 차단 원칙)**
  - [x] `list_events` / `list_documents` / `list_checklists` / `list_projects` / `search_*` 히든 프로젝트 멤버십 필터 — 기존 db 헬퍼들이 처리 (이전 사이클 흡수)
  - [x] `get_*` 단건 조회도 동일 필터 — `_can_read_doc` / `_can_read_checklist` 및 db.get_event_for_mcp 가 work_team_ids/viewer 인자로 처리
  - [x] `team_id` 파라미터 강제에도 히든 멤버십 검증 적용 — `_mcp_work_team_ids` 가 비소속 팀 명시 시 빈 set 반환 → 데이터 없음
- [x] **마이그레이션 (Phase 2에서 처리)**
  - [x] admin의 기존 `mcp_token_hash` 및 `mcp_token_created_at` 둘 다 NULL 초기화 (`team_phase_c22_admin_mcp_token_revoke_v1` phase)
- [x] **검증**
  - [x] admin 세션에서 토큰 발급 UI 안 보임 + API 403 (phase94 B2 + S5)
  - [x] 일반 사용자 MCP에서 다중 팀 데이터 통합 조회 (phase94 B4 — `_mcp_work_team_ids` 가 모든 소속 팀 반환)
  - [x] 같은 팀 멤버이지만 히든 프로젝트 비멤버 — 기존 헬퍼의 `viewer` + `work_team_ids` 필터 적용 (phase85 hidden_project_multiteam 이 검증)
  - [x] `get_project(id=히든)` 비멤버 → 기존 헬퍼 정책 적용 (별도 검증은 phase85 영역)
...
- [x] **구현 (soft delete + 유예기간)**
  - [x] `/admin`에서만 팀 삭제·복원 (팀 관리자 차단)
  - [x] 삭제 시 `teams.deleted_at` 기록 (즉시 접근 차단)
  - [x] 유예기간 중 `/팀이름` 접속 → "팀 삭제 예정" 안내 페이지 (계정 가입·팀 신청·공개 데이터 모두 비노출)
  - [x] 유예기간 중인 팀은 신규 팀 신청 후보 제외
  - [x] 대표 팀/현재 작업 팀 후보에서 삭제 예정 팀 제외
  - [x] 사용자가 삭제 예정 팀만 남으면 팀 미배정처럼 처리
  - [x] **유예기간 중 `user_teams` row는 삭제하지 않고 그대로 유지** — 접근 차단은 서버에서 `teams.deleted_at IS NOT NULL`로 처리, 복원 시 멤버 관계 자동 복구
  - [x] 시스템 관리자 복원 기능
- [x] **구현 (자동 완전 삭제 스케줄러)**
  - [x] 매일 새벽 DB 백업 완료 이후 실행
  - [x] 90일 경과한 팀의 데이터 삭제 — **명시적 순서 + 테이블 목록**:
    ...
    12. **팀 본체**: `teams`
  - [x] 누락 위험 줄이기: 위 목록을 단일 함수에 상수로 정의하고, 신규 팀 종속 테이블 추가 시 이 목록도 같이 갱신 → `database.py:_TEAM_PURGE_STEPS` 상수 + `_purge_team_data` 본문 단일 함수
  - [x] **구현 직전 `database.py` / `sqlite_master` 기준 최종 대조** — feature_spec.md 사전 점검에서 9개 명시 + 6개 간접 참조 테이블 모두 12단계 커버 확인. 누락 0건
  - [x] **업로드 폴더는 즉시 물리 삭제하지 않는다** (`uploads/teams/{team_id}/...` + `/uploads/meetings/...` 모두) — `_purge_team_data`에서 파일 시스템 작업 0, 별도 고아 파일 정리 작업이 처리할 책임
- [x] **검증**
  - [x] 팀 삭제 → 즉시 접근 차단 + 안내 페이지 (#10/#13 + `_can_read_doc`/`auth.user_team_ids` deleted_at 필터 + team_portal.html deleted 분기)
  - [x] 복원 → `user_teams`가 보존되어 있어 모든 데이터·멤버 관계 자동 복구 (test_B8)
  - [x] 90일 경과 팀 → 자동 완전 삭제 + 같은 이름 재사용 가능 (test_B9, B11)
  - [x] notifications row가 events 삭제 전에 먼저 정리되어 dangling 포인터 없음 (test_S3, test_B10)
...
- [x] **구현**
  - [x] 문서(meetings) 첨부는 기존 `/uploads/meetings/YYYY/MM/...` 유지 (doc_editor.html / notice.html endpoint 변경 0)
  - [x] 체크·일정 등 신규 첨부는 `uploads/teams/{team_id}/...` 분리 (check_editor.html / check.html `?kind=team`, events 첨부 UI 부재라 subdir만 정의)
  - [x] 문서 외 업로드 API에 `team_id` 확정/권한 검증 추가 (`_resolve_upload_target` + `auth.resolve_work_team`, None이면 400)
  - [x] 다운로드는 raw `StaticFiles` 공개가 아니라 DB 참조 권한 검증 (`_ProtectedTeamStaticFiles` 신규, `_can_read_uploaded_file` 재사용 — `find_upload_references` LIKE %url% 매칭이라 URL 패턴 무관 자동 커버)
- [x] **검증**
  - [x] 새 체크 첨부가 `uploads/teams/{team_id}/checks/` 아래 저장 (test_B2 디스크 파일 존재 검증)
  - [x] 다른 팀 멤버가 직접 URL로 첨부 접근 시도 → 차단 (test_B6 권한 우회 시나리오 — `_ProtectedTeamStaticFiles` → `_can_read_uploaded_file` → `_can_read_checklist` 다른 팀 False → 404)
  - [x] 기존 `/uploads/meetings/...` 파일 정상 다운로드 (test_B1 default kind=meeting + 회귀 phase89~95 누적 107 PASS — 변경된 라우트는 backward-compat default)
...
- [x] 비로그인 화면 v2 재설계 catchup (2026-05-15) — 그룹 B 화면 재설계 v2. `/팀/{칸반|간트|문서|체크}` 4개 비로그인 페이지를 별도 `portal-panel` 마크업이 아닌 **로그인 정식 템플릿 재활용 + `_ctx().user=None` 마스킹** 방식으로 재구성(`.kanban-toolbar`/`.project-toolbar`/`.doc-table`/`.ck-page` 정식 마커 렌더, `.portal-panel` 잔존 0). 본문 액션 진입점은 `IS_LOGGED_IN=false` 결정적 마스킹으로 일괄 차단, 헤더 우측은 `로그인` 1개만 노출, AI 주간보고 트리거 0. 신규 공개 API 3종 `/api/public/teams/{team_id}/{kanban,project-timeline,checklists}` 분리(openapi 등록, GET 200, mutating 405), `window.PORTAL={public:true,teamId,teamName}` 주입 + `current-user-json=null`. `/AI` 랜딩 v1 유지(`menu_key is None` 분기), `/` 미니멀 진입 유지. `tests/phase101_unauth_redesign_v2.spec.js` 18/18 PASS, 로그인 7페이지 회귀 0(PORTAL=null, pageerror 0, 5xx 0). advisor 2회 컨설팅 반영. **별건 추적**: `/admin/teams` 양쪽 origin 404(다음 catchup), 비로그인 체크 좌측 프로젝트 `???` 인코딩 의심, baseline 회귀 20건 유지.
...
| 2026-05-14 | #22 MCP 팀 범위 + admin 토큰 차단 (그룹 C 일곱 번째·마지막) | 1회성 마이그레이션 phase `team_phase_c22_admin_mcp_token_revoke_v1` 추가 — `UPDATE users SET mcp_token_hash=NULL, mcp_token_created_at=NULL WHERE role='admin'` (둘 다 NULL 필수: hash만 비우면 UI 토큰 보유 표시 잔존, `migration_phase:` prefix 마커로 idempotent). `app.py POST /api/me/mcp-token/regenerate` 첫 줄 `is_admin → 403 "시스템 관리자는 MCP 토큰을 발급할 수 없습니다."` 가드. `mcp_server.py _mcp_work_team_ids(user, requested_team_id=None)` 시그니처 확장 — 미명시 base(모든 소속 팀), 명시 + admin → `{tid}`, 명시 + 비admin 소속 → `{tid}`, 명시 + 비admin 비소속 → `set()` (빈 결과로 안전 차단). 4 tools 시그니처 `team_id: int|None=None` 추가(list_projects/list_events/list_documents/list_checklists). 나머지 10 tools (get_*/search_*) 는 본 사이클 미변경 — 통합 조회 유지, team 파라미터 일관화는 별건. `templates/settings_mcp.html` admin 분기 — 토큰 발급/재발급/삭제 버튼 비노출 + 안내 문구 "(계획서 §16-1)". 결과 row 의 team_id/team_name 동반은 기존 `get_all_meetings_summary` LEFT JOIN teams 등에서 이미 적용 — 본 사이클 별도 작업 0건(흡수). 일반 사용자/멤버 조회 도구는 현재 14 tools 에 없음(흡수 메모). 히든 프로젝트 우회 차단은 기존 db 헬퍼들(`_can_read_doc`/`_can_read_checklist`/`get_event_for_mcp` 등)이 `viewer`+`work_team_ids` 인자로 처리. 리뷰: 차단 0, 경고 2(10 tools 의 team 파라미터 일관화는 별건, 마이그레이션 phase 후 admin 토큰 재발생 경로는 라우트 가드가 1차 보호). **QA: TestClient + 임시 DB** — `tests/phase94_mcp_admin_block_and_team_scope.py` 11/11 PASS(정적 S1~S5+S_import / 행동 B1~B5: phase 마커 재실행으로 admin 토큰 NULL, admin POST 403, 일반 사용자 200, 비admin 소속 팀 명시 → 단일 set, 비소속 → 빈 set). 회귀 phase89/90/91/92/93 = 74/74, 누적 85/85. 인시던트 1건(테스트 marker key `_` vs `:` prefix 오타) 즉시 해소·프로덕션 무변경. **서버 재시작 필요** (phase 자동 실행 + 라우트 가드 + mcp_server.py reload). 범위 밖(불변): 10 tools team 파라미터 일관화(별건), 사용자 리스팅 도구 부재(흡수). **그룹 C 마무리 — #16~#22 모두 닫힘**. | `database.py`, `app.py`, `mcp_server.py`, `templates/settings_mcp.html`, `tests/phase94_mcp_admin_block_and_team_scope.py`(신규), `팀 기능 구현 todo.md`. workspace: 동일 5종.
| 2026-05-14 | #21 알림 통합 표시 + 작업 팀 자동 전환 (그룹 C 여섯 번째) | 스키마 무변경 — `notifications.team_id` 컬럼·#4 백필 모두 완료 상태. **결함 5개 발견·해소**: ① `create_notification`/`for_all`/`for_team` INSERT 가 team_id 컬럼 누락(신규 알림 항상 NULL 저장) → 시그니처에 `team_id=None` 추가 + INSERT 컬럼 보강(backward compat). ② `get_pending_notifications` 가 단순 `user_name=?`만 검사 → viewer 가 admin 아니면 `auth.user_team_ids(viewer)` 위임으로 멤버십 필터(`team_id IS NULL OR team_id IN allowed`), admin 은 무필터. ③ team_name 미동반 → LEFT JOIN teams + `t.name AS team_name` 컬럼 추가. ④ 알림 클릭 작업 팀 전환 미구현 → `templates/base.html`: `_renderNotifList` 가 `team_name` 뱃지 + onclick(`onNotifClick`), 신규 함수 `window.onNotifClick` 이 기존 `POST /api/me/work-team` (#15) 재사용 + 실패 시 toast. team_id NULL 알림은 onclick 미부착(전역 알림). ⑤ `auth.user_team_ids` 가 이미 `status='approved' AND teams.deleted_at IS NULL` 필터 적용(auth.py:64) — 추가 작업 불필요. 회귀 안전 (default None backward compat). 리뷰: 차단 0, 경고 2(.notif-team CSS 미정의 텍스트 prefix 로 기능 OK, create_notification_for_all 의 team_id 명시는 향후 별건). **QA: TestClient + 임시 DB** — `tests/phase93_notifications_multiteam.py` 11/11 PASS(정적 S1~S4 / 행동 B1~B7: 두 팀·추방 row 보존·NULL 항상 노출·admin 무필터·INSERT team_id·team_name 동반). 회귀 phase89/90/91/92 = 63/63, 누적 74/74. 인시던트 2건(테스트 anchor 짧음 + 한글 팀명 정규식 위반) 즉시 해소·프로덕션 무변경. **서버 재시작 필요** (database.py reload). 범위 밖(불변): assignee 변경 등 호출처에 team_id 명시 전달은 향후 점진적(default None backward compat 으로 기존 호출 안전), `.notif-team` CSS 디자인. | `database.py`, `templates/base.html`, `tests/phase93_notifications_multiteam.py`(신규), `팀 기능 구현 todo.md`. workspace: 동일 5종.
| 2026-05-14 | #24 첨부파일 저장 정책 분기 (그룹 D 두 번째·마지막) | 스키마 phase 추가 없음 — 디스크 디렉터리만(자동 mkdir). app.py: `TEAMS_UPLOAD_DIR = _RUN_DIR/uploads/teams`(mkdir parents+exist_ok) + `_upload_url_from_team_static_path` 헬퍼 + `_ProtectedTeamStaticFiles` 클래스(`_ProtectedMeetingStaticFiles`와 동일 골격, `_can_read_uploaded_file` 재사용) + `app.mount("/uploads/teams", ...)`. 라우트 2개에 `kind: str = "meeting"` query param 추가 + 신규 `_resolve_upload_target(request, user, kind, subdir) -> (folder, url_prefix)` 헬퍼(kind=meeting 기존 동작 100% 보존 / kind=team → `auth.resolve_work_team` 호출 후 None이면 400, 성공 시 `TEAMS_UPLOAD_DIR/{tid}/{subdir}/{YYYY}/{MM}` + `/uploads/teams/{tid}/{subdir}/{YYYY}/{MM}` / 그 외 kind → 400 allowlist). 두 라우트(image/attachment) × 두 경로(in-process fallback / Media service IPC) 모두 `folder`/`url_prefix` 변수 사용으로 일괄 교체. `templates/check_editor.html`: imageUpload endpoint + attachment fetch 모두 `?kind=team`. `templates/check.html`: imageUpload endpoint `?kind=team`. **doc_editor.html / notice.html / `/uploads/meetings/` 라우트는 무변경** (계획서 §14 line 823: 문서는 meetings 정책 유지). **레거시 체크 첨부 파일 물리 이동 X**(계획서 line 847). advisor 사전 점검: `find_upload_references`(database.py:3561) 가 `LIKE %url%` 매칭이라 URL 패턴 무관 → 신규 team URL 도 자동 검색 → `_can_read_uploaded_file`/`_can_read_checklist` 권한 체인 그대로 적용 → 권한 우회 시나리오 미발생(B5+B6 직접 검증). 리뷰: 차단 0, 경고 2(`_IMG_URL_RE`(app.py:3507)가 `/uploads/meetings/`로 hardcoded → 신규 team 경로 이미지 cleanup 누락은 별건 후속 / notice.html 미변경은 별건). **QA: TestClient + 임시 DB + tempdir + 실 PNG** — `tests/phase96_team_upload_split.py` 13/13 PASS(정적 S1~S5+S_import / 행동 B1~B7: kind=meeting default · kind=team 저장 · 미배정 사용자 400 · invalid kind 400 · find_upload_references team URL 매칭 · 다른 팀 멤버 404 · 같은 팀 멤버 200). 인시던트 1건(S2 정규식 multiline `[^)]+` → `(.*?)\):`로 변경) 즉시 해소·프로덕션 무변경. 회귀 phase89~95 = 107/107, 누적 120/120. **서버 재시작 필요**(uvicorn reload — 새 mount + 라우트 시그니처 변경). 범위 밖(불변): `_IMG_URL_RE` 패턴 확장(별건), notice.html(별건), events 첨부 UI 부재(YAGNI — `events/` subdir 미생성), Media service IPC 실 호출 검증(in-process fallback만 실 PNG, IPC 경로는 정적 검증). **그룹 D 마무리 — #23~#24 모두 닫힘.** | `app.py`, `templates/check_editor.html`, `templates/check.html`, `tests/phase96_team_upload_split.py`(신규), `팀 기능 구현 todo.md`. workspace: `00_input/feature_spec_24.md, backend_changes_24.md, frontend_changes_24.md, code_review_report_24.md, qa_report_24.md` (+ #23 사이클 파일 5종).
| 2026-05-14 | #23 팀 soft delete + 90일 유예 + 자동 완전 삭제 (그룹 D 첫 번째) | 스키마 phase 추가 없음 — `teams.deleted_at` 컬럼·`get_visible_teams`/`auth.user_team_ids` deleted_at 필터·`team_portal.html {% if deleted %}` 분기 모두 #2/#10/#11/#13에서 이미 완료(advisor 사전 점검). 본 사이클 net-new: ① `database.py`: `soft_delete_team(team_id, *, actor_id=None) -> {team_id, deleted_at, purge_at}`(`UPDATE teams SET deleted_at=CURRENT_TIMESTAMP`, already_deleted/not_found ValueError) + `restore_team` + `get_team_deletion_status` + `list_deleted_teams` + `_purge_at_from`(ISO+90d) + `_TEAM_PURGE_STEPS`(12단계 라벨 상수 = 계약) + `_purge_team_data(conn, team_id)`(16개 DELETE SQL, notifications→histories→locks→trash_refs(SELECT only)→bodies→project_sub→projects→links→team_notices→team_menu_settings→user_teams→teams) + `purge_expired_teams(grace_days=90, *, now=None) -> {purged_teams, errors, rows_deleted_per_step, cutoff}`(팀당 독립 `with get_conn(): BEGIN IMMEDIATE/COMMIT/ROLLBACK` — 한 팀 실패가 다른 팀 막지 않음, now 주입 가능). 기존 `delete_team` 본문 보존 + docstring deprecate 표시(외부 도구 호환·코드 베이스 호출 0건 grep 확인). `apply_to_team`에 사전 가드 추가(`teams.deleted_at IS NOT NULL` → blocked/team_deleted, `get_team_active` 1차 가드의 2차 방어). ② `app.py`: `DELETE /api/admin/teams/{id}` 본문 `db.soft_delete_team` 호출로 교체(이전 hard delete 폐기), `POST /api/admin/teams/{id}/restore` 신규, `GET /api/admin/teams/deleted` 신규. 모두 `_require_admin`(CSRF 통합)·ValueError 매핑. ③ `scheduler_service.py`: `daily-team-purge` cron at **03:40**(03:00 백업 + 03:10 backup cleanup + 03:20 trash + 03:30 orphan image 후, 백업 안전 확보 시점에 hard delete). ④ `templates/admin.html`: 팀 테이블 헤더에 "상태" 컬럼 추가, 활성/삭제 예정 분기 렌더, `_renderDeletedTeamRow`/`restoreTeam` JS 함수 신규, `deleteTeam` confirm 메시지 90일 안내, `badge-warn`/`badge-ok` 인라인 CSS. **`team_portal.html` 안내 페이지는 #13에서 이미 작성됨 → 신규 템플릿 미작성**. 12단계 sqlite_master 대조: `team_id` 컬럼 9개 명시 + 6개 간접 참조 모두 12단계 커버, 누락 0. 리뷰 단계 자가 발견 결함 1건(`get_conn()` 직접 호출 — `@contextmanager`라 generator 반환 → AttributeError) `with` 패턴으로 즉시 패치. 리뷰: 차단 0, 경고 2(`datetime.utcnow()` deprecation은 코드베이스 전반 별건, `delete_team` 본문 보존). **QA: TestClient + 임시 DB** — `tests/phase95_team_soft_delete.py` 22/22 PASS(정적 S1~S7+S_import / 행동 B1~B14: soft/restore/status/list/apply_to_deleted/user_teams 보존/cutoff/notifications-before-events/12단계 완전 정리/route 200·404·400·403). 인시던트 3건(notifications.user_name·payload→message·checklist_histories.content_snapshot→content·session 쿠키 `wu_sid`→`session_id`) 모두 테스트 코드 수정으로 해소·프로덕션 무변경. 회귀 phase89/90/91/92/93/94 = 85/85, 누적 107/107. **서버 재시작 필요**(app·database 모듈 + scheduler 사이드카 reload — 03:40 cron 등록을 위해 사이드카 재시작 필수, phase 추가 없음). 범위 밖(불변): 업로드 폴더 즉시 삭제 X(별도 고아 파일 정리 책임), `delete_team` hard 함수 잔존(외부 도구 호환). | `database.py`, `app.py`, `scheduler_service.py`, `templates/admin.html`, `tests/phase95_team_soft_delete.py`(신규), `팀 기능 구현 todo.md`. workspace: `00_input/feature_spec.md, backend_changes.md, frontend_changes.md, code_review_report.md, qa_report.md`.
